...
class GoogleDistanceMatrix:
    def __init__(self, subscription_key: str="") -> None:
        ...
        self.data =  pd.read_csv('./data/googleDistanceMatrix/distance.csv')
    ...
    def run(self, origin, destination, mode='driving'):
        ...
        if len(response) > 0:
                ...
                return {
                    "status": "success",
                    "result": f"{mode}, from {origin} to {destination}, duration: {info['duration']}, distance: {info['distance']}, cost: {info['cost']}"
                }

        # return f"{mode}, from {origin} to {destination}, no valid information."   
        return {
            "status": "error",
            "result": f"{mode}, from {origin} to {destination}, no valid information."
        }
